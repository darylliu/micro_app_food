...
Page({
  ...
  bindClearHistory() {
    ...
    db.collection('historys').doc('history' + this.data.openid).update({
      data: {
        historyList: []
      }
    }).then((res) => {
    ...
    db.collection('historys').where({
      ...
      success: res => {
        ...
        if (!res.data.length) {
          ...
          db.collection('historys').doc('history' + that.data.openid).update({
            data: {
  ...
  getHistory() {
    let that = this
    const db = wx.cloud.database()
    db.collection('historys').doc('history' + that.data.openid).get({
      success(res) {
